...
use Koldy\Db\Query\{Delete, Insert, Select, Update};
use Koldy\Exception;
use Koldy\Log;
...
class Db extends AbstractCacheAdapter
{
	...
	public function set(string $key, mixed $value, int|null $seconds = null): void
	{
		...
		try {
			...
			throw new CacheException("Couldn't store value(s) to cache key \"{$key}\" because it failed on database level: {$e->getMessage()}",
				$e->getCode(), $e);
		}
	}
	...
	public function delete(string $key): void
	{
		try {
			...
			throw new CacheException("Couldn't delete cache key \"{$key}\" because it failed on database level: {$e->getMessage()}",
				$e->getCode(), $e);
		}
	}
	...
	public function deleteAll(): void
	{
		try {
			...
			throw new CacheException("Couldn't delete all cache keys because it failed on database level: {$e->getMessage()}",
				$e->getCode(), $e);
		}
	}
	...
	public function deleteMulti(array $keys): void
	{
		try {
			...
			$keys = implode(', ', $keys);
			throw new CacheException("Couldn't delete multiple cache keys ({$keys}) because it failed on database level",
				$e->getCode(), $e);
		}
	}
