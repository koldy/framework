	...
	public function prepareHttp(string $uri): void
	{
		...
		if ($this->uriParts[1] == '') {
			...
			$this->controllerClass = str_replace(' ', '',
					ucwords(str_replace(['-', '.'], ' ', $this->controllerUrl))) . 'Controller';
		}
		...
		if (is_dir($moduleDir)) {
			...
			if ($mainControllerExists) {
				if (!isset($this->uriParts[3]) || $this->uriParts[3] == '') {
					...
					$this->actionMethod = strtolower(substr($this->actionMethod, 0, 1)) . substr($this->actionMethod,
							1);
				}
			...
			Application::addIncludePath($basePath . 'controllers',
				// so you can extend abstract controllers in the same directory if needed,
				$basePath . 'library'      // the place where you can define your own classes and methods
			);
		}

		$this->isAjax = (isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] == 'POST' && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') || (isset($_SERVER['HTTP_ACCEPT']) && str_contains($_SERVER['HTTP_ACCEPT'],
					'application/json'));
		...
		if ($alwaysRestful || (property_exists($controllerInstance,
					'restful') && $controllerInstance::$restful === true)) {
			// it is restful
