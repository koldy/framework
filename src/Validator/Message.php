...
class Message
{
	...
	protected static array $messages = [
		self::PRESENT => 'Parameter {param} is not present',
		// {param}, {value}
		self::REQUIRED => 'This field is required',
		// {param}, {value}
		self::MIN_VALUE => 'Has to be at least {min}',
		// {param}, {value}, {min}
		self::MAX_VALUE => 'Has to be less than {max}',
		// {param}, {value}, {max}
		self::MIN_LENGTH => 'Has to be at least {min} characters long',
		// {param}, {value}, {min}
		self::MAX_LENGTH => 'Has to be less than {max} characters long',
		// {param}, {value}, {max}
		self::NUMERIC => 'Has to be numeric',
		// {param}, {value}
		self::PRIMITIVE => '{param} has to be primitive value, not array nor object',
		// {param}
		self::LENGTH => 'This field has to be exactly {length} characters long',
		// {param}, {value}
		self::INTEGER => 'This field has to be integer',
		// {param}, {value}
		self::ALPHA_NUM => 'This field is not alpha numeric',
		// {param}, {value}
		self::ALPHA => 'This field does not contain alpha characters only',
		// {param}, {value}
		self::EMAIL => 'This is not valid e-mail address',
		// {param}, {value}
		self::SLUG => 'This is not valid URL slug',
		// {param}, {value}
		self::IS => 'This field doesn\'t have required value',
		// {param}, {value}
		self::DECIMAL => 'This value can\'t have more than {decimals} decimals',
		// {param}, {value}, {decimals}
		self::HEX => 'This value should be hexadecimal number',
		// {param}, {value}
		self::SAME => 'This has to be the same as {otherField}',
		// {param}, {value}, {otherField}, {otherValue}
		self::DIFFERENT => 'This value has to be different than {otherField}',
		// {param}, {value}, {otherField}, {otherValue}
		self::DATE => 'This value is not valid date',
		// {param}, {value}
		self::IS_NOT_ARRAY => '{param} is not array',
		// {param}
		self::ARRAY_WRONG_COUNT => '{param} should have {requiredCount} elements, not {currentCount}',
		// {param}, {requiredCount}, {currentCount}
		self::ANY_OF => 'This field doesn\'t have any allowed value',
		// {param}, {value}, {allowedValues}
		self::NOT_UNIQUE => 'We already have this',
		// {param}, {value}, {exceptionValue}, {exceptionField}
		self::NO_RECORD => 'This value does not exists in database',
		// {param}, {value}, {field}
		self::CSRF_FAILED => 'CSRF check has failed',
		// {param}
		self::BOOL => 'Parameter is not boolean',
		// {param}
		self::STARTS_WITH => 'Value should start with {startsWith}',
		// {param}, {value}, {startsWith}
		self::ENDS_WITH => 'Value should end with {endsWith}',
		// {param}, {value}, {endsWith}
		self::UUID => 'Value doesn\'t have correct UUID format'
		// {param}, {value}
	];
	...
	public static function getMessage(int $constant, array|null $data = null): string
	{
		...
		if ($msg instanceof Closure) {
			try {
				...
				throw new Exception("Failed to execute custom validator function: {$e->getMessage()}", $e->getCode(),
					$e);
			}
	 ...
	 *
	 * @param int $constant
	 * @param Closure $userFunction The user function must return string, otherwise \Koldy\Validator\Exception will be
	 *     thrown
	 */
