...
use Koldy\Db\Query\{Delete, Insert, ResultSet, Select, Update};
use Koldy\Json;
use Stringable;
use TypeError;
...
abstract class Model implements Stringable
{
	...
	/**
	 * The DB adapter key from config on which the queries will be executed. Null means default adapter (or the first
	 * from configs/database.php)
	 *
	 * @var string|null
	 */
	 ...
	 * name by the called class name. Class \Db\User\Roles that extends Model class will try to use db_user_roles table
	 * in DB
	 *
	 * @var string|null
	 */
		...
		if (static::$autoIncrement) {
			...
			if (is_string(static::$primaryKey) && isset($data[static::$primaryKey])) {
				...
				try {
					...
					throw new Exception("Unable to get last insert ID; please check if {$thisClass} should have incrementable primary key",
						$e->getCode(), $e);
				}
			}
		}
	 ...
	 */
	public static function fetchOneOrFail(
		int|float|string|array|Where $field,
	 ...
	 * @throws \Koldy\Config\Exception
	 * @throws \Koldy\Exception
	 * @example User::fetchArrayOf('id', Where::init()->where('id', '>', 50), 'id', 'asc') would return
	 ...
	 * @throws \Koldy\Exception
	 */
	public static function fetchOneValueOrFail(
		string $field,
		int|float|string|array|Where|null $where = null,
		string|null $orderField = null,
		string|null $orderDirection = null
	 ...
	 * @throws \Koldy\Exception
	 */
	public static function fetchOneValue(
		string $field,
		int|float|string|array|Where|null $where = null,
		string|null $orderField = null,
		string|null $orderDirection = null
	 ...
	 * contains multiple columns, then all columns must be present in the model in order to refresh the data
	 * successfully.
	 *
	 * @return static
	 * @throws Exception
