...
use Koldy\Db\Adapter\{MySQL, PostgreSQL, Sqlite};
			...
			if (count($files) === 0) {
				...
				if ($wasntExecutedBeforeCount > 0) {
					...
					if ($force) {
						...
						Log::info("{$wasntExecutedBeforeCount} migration{$s} WILL NOT be executed because those files are not in the right order and script is not forced (no --force flag). Not-executed migration{$s} from before are:",
							implode(', ', $notExecutedFilesBefore));
					}
				}
