...
class Util
{
	...
	public static function truncate(
		...
		if (mb_strlen($string, $encoding) > $length) {
			...
			if (!$middle) {
				...
				return mb_substr($string, 0, (int)round($length / 2), $encoding) . $etc . mb_substr($string,
						(int)round(-$length / 2), null, $encoding);
			}
		} else {
			return $string;
		}
	 ...
	 *
	 * @return string
	 * @example text "Lorem ipsum\n\ndolor sit amet\nperiod." will become "<p>Lorem ipsum</p><p>dolor sit
	 *     amet<br/>period.</p>"
	 */
	...
	{
		return mb_substr($yourString, 0, mb_strlen($startsWith, $encoding ?? Application::getEncoding()),
				$encoding ?? Application::getEncoding()) === $startsWith;
	}
	...
	{
		return mb_substr($yourString, 0 - mb_strlen($endsWith, $encoding ?? Application::getEncoding()), null,
				$encoding ?? Application::getEncoding()) === $endsWith;
	}
	...
	public static function camelCase(
		string $string,
		array|null $noStrip = null,
		bool|null $lowerCaseFirstLetter = null
	): string {
	 ...
	 *
	 * @param string $input the whole multipart content with boundary marks, that one that can be got with
	...
	/**
	 * Pick the value from array by key. If key doesn't exist, return default value. If key exists, but value is null,
	 * return default value.
	 *
	 * @param string $key name of the key to pick from the input array
	 * @param array $input the input array to go through
	 * @param string|int|float|bool|null $default default value to return if key doesn't exist or value is null;
	 *     default is null
	 * @param array|null $allowedValues if set, then only values from this array are allowed; if value is not in this
	 *     array, then default value is returned; values will be strictly compared
	 *
	 * @return mixed
	 */
