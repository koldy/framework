...
use Koldy\Validator\{ConfigException,
	ConfigException as ValidatorConfigException,
	Exception as InvalidDataException,
	Message,
	Validate};
use stdClass;
...
class Validator
{
	...
	/**
	 * Make validation more strict by setting this to true - if true, we'll expect parameters only from rules, nothing
	 * less, nothing more
	 *
	 * @var bool
	 */
	 ...
	 *
	 * @var bool
	 * @deprecated Don't rely on CSRF validation here as it's deprecated and will be removed in future releases. To
	 *     avoid any issues in the future, disable CSRF check in application config.
	 */
	 ...
	 * @param array|null $data - the data you want to validate with a rules; if not set, data will be taken from
	...
	public static function create(
		...
		bool $validateAndThrowException = true
	): Validator {
	...
	/**
	 * Get the list of rules where comma doesn't mean it is argument separator. These rules usually can have only one
	 * argument, so it's ok
	 *
	 * @return array
	 */
	 ...
	 * @deprecated Don't rely on CSRF validation here as it's deprecated. To avoid any issues in the future, disable
	 *     CSRF check in application config.
	 */
	...
	public function getData(bool|null $trimStrings = null): array
	{
		...
		foreach ($this->rules as $field => $rules) {
			...
			if (isset($data[$field]) && is_string($data[$field]) && $rules !== null) {
				$rules = explode('|',
					is_object($rules) && method_exists($rules, '__toString') ? $rules->__toString() : $rules);
	 ...
	 * data being validated or if passed value is empty string or if it's null (not string 'null', but real null for
	 * some reason)
	 *
	 * @param mixed $value
	 ...
	 */
	protected function validateBool(
		mixed $value,
		string $parameter,
		array $args = [],
		array|null $rules = null,
	 ...
	 */
	protected function validateDecimal(
		mixed $value,
		string $parameter,
		array $args = [],
		array|null $rules = null,
	 ...
	 *  'param2' => 'same:param1'
	 */
	protected function validateSame(
		mixed $value,
		string $parameter,
		array $args = [],
		array|null $rules = null,
	 ...
	 */
	protected function validatePresent(
		mixed $value,
		string $parameter,
		array $args = [],
		array|null $rules = null,
	 ...
	 */
	protected function validateDifferent(
		mixed $value,
		string $parameter,
		array $args = [],
		array|null $rules = null,
	 ...
	 * 'email' => 'email|unique:\Db\User,email,field:id,id' // check if email exists in \Db\User model, but exclude ID
	 *     with value from param 'id'
	 */
	 ...
	 * @throws InvalidDataException
	 * @throws ValidatorConfigException
	 * @example 'user_id' => 'required|integer|exists:\Db\User,id' // e.g. user_id = 5, so this will check if there is
	 *     record in \Db\User model under id=5
	 */
	protected function validateExists(
		mixed $value,
		string $parameter,
		array $args = [],
		array|null $rules = null,
	 ...
	 */
	protected function validateEndsWith(
		mixed $value,
		string $parameter,
		array $args = [],
		array|null $rules = null,
