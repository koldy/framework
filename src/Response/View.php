...
class View extends AbstractResponse
{
	...
	protected function getViewPath(string $view): string
	{
		...
		if ($pos === false) {
			...
			return dirname(substr($this->viewPath, 0, -1)) . DS . 'modules' . DS . substr($view, 0,
					$pos) . DS . 'views' . DS . substr($view, $pos + 1) . '.phtml';
		}
	}
	...
	public static function exists(string $view): bool
	{
		...
		if ($pos === false) {
			...
			$path = dirname(substr(Application::getViewPath(), 0, -1)) . DS . 'modules' . DS . substr($view, 0,
					$pos) . DS . 'views' . DS . substr($view, $pos + 1) . '.phtml';
		}
