...
class Application
{
	 ...
	 * - development/live - (running live, but in dev mode; e.g. you want your analytics, but serving dev not minimized
	 * versions)
	...
	/**
	 * The name of the app. Can be set to anything. Not required, but sometimes useful when working with huge
	 * multi-domain system.
	 *
	 * @var string|null
	 * @example website
	...
	/**
	 * The array of functions that will be executed after any response is sent to client. First argument is the
	 * response object or any other possible response from the application (such as simple string)
	 *
	 * @var Closure[]
	 */
	...
	/**
	 * The array of function names that will be executed after any response is sent to client - when registering a
	 * function, you can register it under a name, so you can check if it's already registered
	 *
	 * @var string[]
	 */
	...
	/**
	 * Set the response from the application. It is used by internal classes, so you should never call this method
	 ...
	 * @example if you call "./bin/phpunit --configuration phpunit.xml", this method will return "./bin/phpunit
	...
	/**
	 * Check if current runtime is running as CLI script. To detect CLI, we should not rely only on php_sapi_name() or
	 * $argv. Solution is taken from StackOverflow.
	 *
	...
	/**
	 * Get the configs from any config file, fetched by config name. Config name is the name on file system or the key
	 * from configs key in index.php.
	 *
	 * There are three possible locations where configs can be stored (by priority and speed of access):
	 * 1. in application's config so it's immediately available (application config -> "configs" key) (in this case,
	 * it's not possible to reload config in runtime)
	 * 2. by saying/overriding the exact path of each config (application config -> "config" key and then the name of
	 * config as key in "config" key)
