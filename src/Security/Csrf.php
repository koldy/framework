...
class Csrf
{
	...
	public static function generate(string|null $token = null, int|null $length = null): Token
	{
		...
		if (is_string($cookieName) && strlen($cookieName) > 0) {
			$cookie = Cookie::rawSet($cookieName, $token, 0, '/', $config->get('domain') ?? '',
				$config->get('cookie_secure') ?? false, false, $config->get('cookie_samesite'));
		} else {
			$cookie = null;
		}
	...
	public static function getMetaTags(): string
	{
		...
		if (($parameterName = static::getParameterName()) !== null) {
			$csrfValue = static::getStoredToken()->getToken();
			return sprintf('<meta name="csrf_name" content="%s"/><meta name="csrf_value" content="%s"/>',
				$parameterName, $csrfValue);
		} else {
			return '';
		}
	}
